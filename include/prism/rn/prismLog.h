...
inline void logToJS(jsi::Runtime &rt, const std::string &msg, const char *file, int line,
...
inline void logToJS(jsi::Runtime &rt, const char *file, int line, LogLevel level, fmt::format_string<Args...> fmtStr,
                    Args &&...args)
{
...
inline void logToJSAsync(jsi::Runtime &rt, std::shared_ptr<facebook::react::CallInvoker> jsCallInvoker,
                         const std::string &msg, const char *file, int line, LogLevel level = LogLevel::Info)
{
    if (!jsCallInvoker)
        return;
...
template <typename... Args>
inline void logToJSAsync(jsi::Runtime &rt, std::shared_ptr<facebook::react::CallInvoker> jsCallInvoker,
                         const char *file, int line, LogLevel level, fmt::format_string<Args...> fmtStr, Args &&...args)
{
    std::string msg = fmt::format(fmtStr, std::forward<Args>(args)...);
...
#define LOG_INFO_F(rt, invoker, fmtStr, ...)                                                                           \
    prism::rn::logToJSAsync(rt, invoker, __FILE__, __LINE__, prism::rn::LogLevel::Info, FMT_STRING(fmtStr),            \
                            ##__VA_ARGS__)

#define LOG_WARN_F(rt, invoker, fmtStr, ...)                                                                           \
    prism::rn::logToJSAsync(rt, invoker, __FILE__, __LINE__, prism::rn::LogLevel::Warn, FMT_STRING(fmtStr),            \
                            ##__VA_ARGS__)

#define LOG_ERROR_F(rt, invoker, fmtStr, ...)                                                                          \
    prism::rn::logToJSAsync(rt, invoker, __FILE__, __LINE__, prism::rn::LogLevel::Error, FMT_STRING(fmtStr),           \
                            ##__VA_ARGS__)
